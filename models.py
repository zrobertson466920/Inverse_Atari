...
def latent_model(learning_rate=0.001, decay=0.0):
    ...
    model.compile(loss=[min_mse,latent_cross(new_image,after_image)], loss_weights = [0.1,0.9], metrics = {'action': latent_acc(new_image,after_image)}, optimizer=Adam(lr=learning_rate, decay=decay))
